...
from analytics_chart import render_downloads_over_time, render_filetype_pie, render_activity_bar
...
class App(ctk.CTk):
    ...
    def _finish_launch(self):
        ...
        self.after(150, self._poll_scrape_queue)
    ...
    def _build_analytics_page(self, parent):
        page, body = self._new_page(
            parent, "Download Analytics",
            "Overview of your download activity and performance.")
        ...
        stat_row.pack(fill="x", pady=(0, 14))
        for i in range(4):
            stat_row.grid_columnconfigure(i, weight=1, uniform="an_stat")

        top_stat_defs = [
            ("total_downloads", "⬇", STAT_BLUE, "Total Downloads", "0"),
            ("images", "🖼", STAT_BLUE, "Images", "0"),
            ("videos", "🎬", STAT_GREEN, "Videos", "0"),
        ...
        charts_row = ctk.CTkFrame(body, fg_color="transparent")
        charts_row.pack(fill="both", expand=True, pady=(0, 14))
        charts_row.grid_columnconfigure(0, weight=2)
        charts_row.grid_columnconfigure(1, weight=1)
        charts_row.grid_columnconfigure(2, weight=1)
        charts_row.grid_rowconfigure(0, weight=1)

        line_card = ctk.CTkFrame(charts_row, corner_radius=12, border_width=1,
                                  border_color=CARD_BORDER, fg_color=CARD_BG)
        line_card.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        ctk.CTkLabel(line_card, text="Downloads Over Time (last 7 days)",
                     font=ctk.CTkFont(weight="bold", size=15)).pack(anchor="w", padx=16, pady=(14, 4))
        self.analytics_line_container = ctk.CTkFrame(line_card, fg_color="transparent")
        self.analytics_line_container.pack(fill="both", expand=True, padx=8, pady=(0, 10))

        pie_card = ctk.CTkFrame(charts_row, corner_radius=12, border_width=1,
                                 border_color=CARD_BORDER, fg_color=CARD_BG)
        pie_card.grid(row=0, column=1, sticky="nsew", padx=(0, 8))
        ctk.CTkLabel(pie_card, text="File Type Distribution",
                     font=ctk.CTkFont(weight="bold", size=15)).pack(anchor="w", padx=16, pady=(14, 4))
        self.analytics_pie_container = ctk.CTkFrame(pie_card, fg_color="transparent")
        self.analytics_pie_container.pack(fill="both", expand=True, padx=8, pady=(0, 10))

        bar_card = ctk.CTkFrame(charts_row, corner_radius=12, border_width=1,
                                 border_color=CARD_BORDER, fg_color=CARD_BG)
        bar_card.grid(row=0, column=2, sticky="nsew")
        ctk.CTkLabel(bar_card, text="Activity by Type (all-time)",
                     font=ctk.CTkFont(weight="bold", size=15)).pack(anchor="w", padx=16, pady=(14, 4))
        self.analytics_bar_container = ctk.CTkFrame(bar_card, fg_color="transparent")
        self.analytics_bar_container.pack(fill="both", expand=True, padx=8, pady=(0, 10))

        bottom_row = ctk.CTkFrame(body, fg_color="transparent")
        bottom_row.pack(fill="x")
        for i in range(4):
            bottom_row.grid_columnconfigure(i, weight=1, uniform="an_bottom")

        bottom_defs = [
            ("avg_speed", "⚡", STAT_GOLD, "Avg. Speed", "0 MB/s"),
            ("success_rate", "✅", STAT_GREEN, "Success Rate", "0%"),
            ("total_size", "💾", STAT_BLUE, "Total Size", "0 MB"),
    ...
    def _refresh_analytics_page(self):
        summary = stats.get_analytics_summary(days=7)

        ft = summary["file_type_totals"]
        self.analytics_stat_labels["total_downloads"].configure(text=f"{summary['total_downloads']:,}")
        self.analytics_stat_labels["images"].configure(text=f"{ft.get('images', 0):,}")
        self.analytics_stat_labels["videos"].configure(text=f"{ft.get('videos', 0):,}")
        self.analytics_stat_labels["audio"].configure(text=f"{ft.get('audio', 0):,}")

        self.analytics_stat_labels["avg_speed"].configure(text=f"{summary['avg_speed_mbps']:.1f} MB/s")
        self.analytics_stat_labels["success_rate"].configure(text=f"{summary['success_rate']:.1f}%")
        total_mb = summary["total_bytes"] / (1024 * 1024)
        size_text = f"{total_mb / 1024:.2f} GB" if total_mb >= 1024 else f"{total_mb:.1f} MB"
        self.analytics_stat_labels["total_size"].configure(text=size_text)
        self.analytics_stat_labels["failed"].configure(text=f"{summary['total_failed']:,}")

        for widget in self.analytics_line_container.winfo_children():
            widget.destroy()
        fig1 = render_downloads_over_time(summary["day_labels"], summary["day_totals"], summary["day_completed"])
        canvas1 = FigureCanvasTkAgg(fig1, master=self.analytics_line_container)
        canvas1.draw()
        canvas1.get_tk_widget().pack(fill="both", expand=True)

        for widget in self.analytics_pie_container.winfo_children():
            widget.destroy()
        fig2 = render_filetype_pie(ft)
        canvas2 = FigureCanvasTkAgg(fig2, master=self.analytics_pie_container)
        canvas2.draw()
        canvas2.get_tk_widget().pack(fill="both", expand=True)

        for widget in self.analytics_bar_container.winfo_children():
            widget.destroy()
        fig3 = render_activity_bar(summary["activity_totals"])
        canvas3 = FigureCanvasTkAgg(fig3, master=self.analytics_bar_container)
        canvas3.draw()
    ...
    def _build_pdf_extractor_page(self, parent):
        ...
        pdf_tabs = ctk.CTkTabview(body, corner_radius=12)
        pdf_tabs.pack(fill="both", expand=True)
        body = pdf_tabs.add("Extract from PDF")
        pdf_tabs.add("Duplicate Finder")
        pdf_tabs.add("Web Table Scraper")
        ...
        self._build_duplicate_finder_tab(pdf_tabs.tab("Duplicate Finder"))
        self._build_web_scraper_tab(pdf_tabs.tab("Web Table Scraper"))

        return page

    # ------------------------------------------------------------------
    # Duplicate Finder tab — scans files/folders for duplicate images,
    # PDFs, Excel rows, and documents. See duplicate_finder.py.
    # ------------------------------------------------------------------
    def _build_duplicate_finder_tab(self, tab):
        tab.grid_columnconfigure(0, weight=1)
        scroll = ctk.CTkScrollableFrame(tab, corner_radius=0, fg_color="transparent")
        scroll.pack(fill="both", expand=True)

        intro = ctk.CTkFrame(scroll, corner_radius=12, border_width=1, border_color=CARD_BORDER, fg_color=CARD_BG)
        intro.pack(fill="x", pady=10)
        ctk.CTkLabel(intro, text="Find Duplicates", font=ctk.CTkFont(weight="bold", size=14)).pack(pady=(12, 4))
        ctk.CTkLabel(intro,
                     text="Scans a folder (and/or specific files) for duplicate images, PDFs, Excel rows, "
                          "and Word/text documents — exact matches plus near-duplicates.",
                     font=ctk.CTkFont(size=11), text_color="gray", wraplength=650,
                     justify="left").pack(anchor="w", padx=16, pady=(0, 12))

        paths_frame = ctk.CTkFrame(scroll, corner_radius=12, border_width=1, border_color=CARD_BORDER, fg_color=CARD_BG)
        paths_frame.pack(fill="x", pady=10)
        ctk.CTkLabel(paths_frame, text="What to Scan", font=ctk.CTkFont(weight="bold", size=14)).pack(pady=(12, 6))
        self.dup_folder_path = self._path_row(paths_frame, "Folder", self._browse_dup_folder)
        ctk.CTkLabel(paths_frame, text="and/or specific files:",
                     font=ctk.CTkFont(size=11), text_color="gray").pack(anchor="w", padx=16, pady=(2, 0))
        files_line = ctk.CTkFrame(paths_frame, fg_color="transparent")
        files_line.pack(fill="x", padx=16, pady=(4, 12))
        self.dup_files_entry = ctk.CTkEntry(files_line, placeholder_text="No files selected")
        self.dup_files_entry.pack(side="left", fill="x", expand=True, padx=(0, 6))
        ctk.CTkButton(files_line, text="Browse", width=90, command=self._browse_dup_files).pack(side="left")
        self._dup_selected_files = []

        opts_frame = ctk.CTkFrame(scroll, corner_radius=12, border_width=1, border_color=CARD_BORDER, fg_color=CARD_BG)
        opts_frame.pack(fill="x", pady=10)
        ctk.CTkLabel(opts_frame, text="Sensitivity", font=ctk.CTkFont(weight="bold", size=14)).pack(pady=(12, 6))
        sim_line = ctk.CTkFrame(opts_frame, fg_color="transparent")
        sim_line.pack(fill="x", padx=16, pady=(0, 12))
        self.dup_similarity_label = ctk.CTkLabel(sim_line, text="Text/row similarity threshold: 90%")
        self.dup_similarity_label.pack(anchor="w")
        self.dup_similarity_slider = ctk.CTkSlider(sim_line, from_=50, to=100, number_of_steps=50,
                                                     command=self._on_dup_similarity_change)
        self.dup_similarity_slider.set(90)
        self.dup_similarity_slider.pack(fill="x", pady=(6, 0))

        self.dup_output_path = self._path_row(scroll, "Save Report As", self._browse_dup_output)

        self.dup_start_btn = ctk.CTkButton(scroll, text="Scan for Duplicates", height=40,
                                            font=ctk.CTkFont(size=14, weight="bold"),
                                            command=self._start_duplicate_scan)
        self.dup_start_btn.pack(pady=(10, 10), fill="x")

        self.dup_progress_bar = ctk.CTkProgressBar(scroll, mode="indeterminate")
        self.dup_progress_bar.pack(fill="x", pady=(0, 4))
        self.dup_log_box = ctk.CTkTextbox(scroll, height=140, corner_radius=8)
        self.dup_log_box.pack(fill="both", expand=True, pady=(6, 10))

    def _browse_dup_folder(self, entry):
        path = filedialog.askdirectory()
        if path:
            entry.delete(0, "end")
            entry.insert(0, path)

    def _browse_dup_files(self):
        paths = filedialog.askopenfilenames(
            filetypes=[("Supported files", "*.jpg *.jpeg *.png *.bmp *.gif *.webp *.tiff "
                                            "*.pdf *.xlsx *.xls *.csv *.docx *.txt"),
                       ("All files", "*.*")])
        if paths:
            self._dup_selected_files = list(paths)
            self.dup_files_entry.delete(0, "end")
            self.dup_files_entry.insert(0, f"{len(paths)} file(s) selected")

    def _browse_dup_output(self, entry):
        path = filedialog.asksaveasfilename(
            defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")],
            initialfile="duplicates_report.xlsx", confirmoverwrite=False)
        if path:
            entry.delete(0, "end")
            entry.insert(0, path)

    def _on_dup_similarity_change(self, value):
        self.dup_similarity_label.configure(text=f"Text/row similarity threshold: {int(value)}%")

    def _start_duplicate_scan(self):
        if self.dup_thread and self.dup_thread.is_alive():
            messagebox.showinfo("Busy", "A duplicate scan is already running.")
            return

        folder = self.dup_folder_path.get().strip()
        paths = ([folder] if folder else []) + list(self._dup_selected_files)
        if not paths:
            messagebox.showerror("Missing info", "Select a folder and/or at least one file to scan.")
            return
        output_path = self.dup_output_path.get().strip()
        if not output_path:
            messagebox.showerror("Missing info", "Please choose where to save the duplicates report.")
            return
        similarity = int(self.dup_similarity_slider.get())

        self.dup_start_btn.configure(state="disabled", text="Scanning...")
        self.dup_progress_bar.start()
        self.dup_log_box.delete("1.0", "end")

        self.dup_thread = threading.Thread(
            target=self._run_duplicate_scan_job, args=(paths, output_path, similarity), daemon=True)
        self.dup_thread.start()

    def _run_duplicate_scan_job(self, paths, output_path, similarity):
        try:
            finder = DuplicateFinder(similarity_threshold=similarity)
            report = finder.scan(paths)
            finder.export_report(report, output_path)
            counts = {k: len(v) for k, v in report.items()}
            summary = ("Done. Found — images: {images}, pdfs: {pdfs}, excel rows: {excel_rows}, "
                       "docs: {docs}. Saved to {out}").format(out=output_path, **counts)
            self.dup_queue.put(("info", summary))
            self.dup_queue.put(("success", output_path))
        except Exception as e:
            self.dup_queue.put(("error", f"Duplicate scan failed: {e}"))
        ...
        try:
            while True:
                ...
                if kind in ("info", "error"):
                    self.dup_log_box.insert("end", f"{payload}\n")
                    self.dup_log_box.see("end")
                elif kind == "success":
                    self._open_output_folder(payload)
                elif kind == "done":
                    self.dup_progress_bar.stop()
                    self.dup_start_btn.configure(state="normal", text="Scan for Duplicates")
        except queue.Empty:
            pass
        self.after(150, self._poll_dup_queue)

    # ------------------------------------------------------------------
    # Web Table Scraper tab — pulls tables, product listings, prices, and
    # contact info from a URL. See web_table_scraper.py.
    # ------------------------------------------------------------------
    def _build_web_scraper_tab(self, tab):
        tab.grid_columnconfigure(0, weight=1)
        scroll = ctk.CTkScrollableFrame(tab, corner_radius=0, fg_color="transparent")
        scroll.pack(fill="both", expand=True)
        ...
        intro = ctk.CTkFrame(scroll, corner_radius=12, border_width=1, border_color=CARD_BORDER, fg_color=CARD_BG)
        intro.pack(fill="x", pady=10)
        ctk.CTkLabel(intro, text="Scrape a Web Page", font=ctk.CTkFont(weight="bold", size=14)).pack(pady=(12, 4))
        ctk.CTkLabel(intro,
                     text="Paste a URL and pull out HTML tables, product listings (name + price), any "
                          "prices found on the page, and contact info (emails/phone numbers).",
                     font=ctk.CTkFont(size=11), text_color="gray", wraplength=650,
                     justify="left").pack(anchor="w", padx=16, pady=(0, 12))

        url_frame = ctk.CTkFrame(scroll, corner_radius=12, border_width=1, border_color=CARD_BORDER, fg_color=CARD_BG)
        url_frame.pack(fill="x", pady=10)
        ctk.CTkLabel(url_frame, text="Source", font=ctk.CTkFont(weight="bold", size=14)).pack(pady=(12, 6))
        self.scrape_url_entry = self._entry_row(url_frame, "URL", "https://")

        self.scrape_output_path = self._path_row(scroll, "Save As", self._browse_scrape_output)

        self.scrape_start_btn = ctk.CTkButton(scroll, text="Scrape Page", height=40,
                                               font=ctk.CTkFont(size=14, weight="bold"),
                                               command=self._start_web_scrape)
        self.scrape_start_btn.pack(pady=(10, 10), fill="x")

        self.scrape_progress_bar = ctk.CTkProgressBar(scroll, mode="indeterminate")
        self.scrape_progress_bar.pack(fill="x", pady=(0, 4))
        self.scrape_log_box = ctk.CTkTextbox(scroll, height=140, corner_radius=8)
        self.scrape_log_box.pack(fill="both", expand=True, pady=(6, 10))

    def _browse_scrape_output(self, entry):
        path = filedialog.asksaveasfilename(
            defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")],
            initialfile="scraped_data.xlsx", confirmoverwrite=False)
        if path:
            entry.delete(0, "end")
            entry.insert(0, path)

    def _start_web_scrape(self):
        if self.scrape_thread and self.scrape_thread.is_alive():
            messagebox.showinfo("Busy", "A scrape job is already running.")
            return

        url = self.scrape_url_entry.get().strip()
        if not url or url == "https://":
            messagebox.showerror("Missing info", "Please enter a URL to scrape.")
            return
        output_path = self.scrape_output_path.get().strip()
        if not output_path:
            messagebox.showerror("Missing info", "Please choose where to save the scraped data.")
            return

        self.scrape_start_btn.configure(state="disabled", text="Scraping...")
        self.scrape_progress_bar.start()
        self.scrape_log_box.delete("1.0", "end")

        self.scrape_thread = threading.Thread(
            target=self._run_web_scrape_job, args=(url, output_path), daemon=True)
        self.scrape_thread.start()

    def _run_web_scrape_job(self, url, output_path):
        try:
            scraper = WebTableScraper()
            data = scraper.scrape(url)
            scraper.export_report(data, output_path)
            counts = {k: len(v) for k, v in data.items()}
            summary = ("Done. Found — tables: {tables}, products: {products}, prices: {prices}, "
                       "contacts: {contacts}. Saved to {out}").format(out=output_path, **counts)
            self.scrape_queue.put(("info", summary))
            self.scrape_queue.put(("success", output_path))
        except Exception as e:
            self.scrape_queue.put(("error", f"Scrape failed: {e}"))
        self.scrape_queue.put(("done", None))

    def _poll_scrape_queue(self):
        try:
            while True:
                kind, payload = self.scrape_queue.get_nowait()
                if kind in ("info", "error"):
                    self.scrape_log_box.insert("end", f"{payload}\n")
                    self.scrape_log_box.see("end")
                elif kind == "success":
                    self._open_output_folder(payload)
                elif kind == "done":
                    self.scrape_progress_bar.stop()
                    self.scrape_start_btn.configure(state="normal", text="Scrape Page")
        except queue.Empty:
            pass
        self.after(150, self._poll_scrape_queue)
    ...
    def _build_ai_assistant_page(self, parent):
        ...
        ctk.CTkButton(out_header, text="Download", width=90, height=26, fg_color=STAT_BLUE,
                      command=self._download_asst_output).pack(side="right", padx=(0, 8))
    ...
    def _download_asst_output(self):
        """Saves the Response box's content to a file. Guesses a sensible
        default extension from the content so SQL, Python, VBA, etc.
        responses save with a matching file type."""
        text = self.asst_output_box.get("1.0", "end").strip()
        if not text:
            messagebox.showinfo("Nothing to save", "Generate a response first, then download it.")
            return

        lowered = text.lower()
        if "```sql" in lowered or lowered.lstrip().startswith(("select ", "with ", "insert ", "update ")):
            default_ext, filetypes = ".sql", [("SQL files", "*.sql"), ("Text files", "*.txt"), ("All files", "*.*")]
        elif "```python" in lowered or "def " in lowered or "import " in lowered:
            default_ext, filetypes = ".py", [("Python files", "*.py"), ("Text files", "*.txt"), ("All files", "*.*")]
        elif "sub " in lowered and "end sub" in lowered:
            default_ext, filetypes = ".bas", [("VBA module", "*.bas"), ("Text files", "*.txt"), ("All files", "*.*")]
        else:
            default_ext, filetypes = ".txt", [("Text files", "*.txt"), ("All files", "*.*")]
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write(text)
            self.asst_status_label.configure(text=f"Saved to {os.path.basename(path)}", text_color="#3ca34d")
        except Exception as e:
