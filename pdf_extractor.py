...
def _build_system_prompt() -> str:
    return (
        ...
        "contain multiple parties (e.g. seller/vendor/supplier/biller on one side, "
        "buyer/customer/bill-to/ship-to on the other). When a requested field "
        "names a party role, first look for that exact label in the text. If it "
        "isn't labelled exactly that way, match it to whichever party is playing "
        "that role in the transaction, not to whichever party's block happens to "
        "come first or is easiest to find — e.g. 'vendor name'/'seller name'/"
        "'supplier name'/'billed by' all refer to the party providing the goods "
        "or services and getting paid, while 'buyer'/'customer'/'bill to'/"
        "'billed to'/'client' refer to the party being charged. Never swap these "
        "two sides, and never return the recipient's details for a field that "
        "asks for the provider's details (or vice versa)."
    )
